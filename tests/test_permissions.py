...
class APISecurityTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user1.groups.add(host_group)
        self.user2.groups.add(guest_group)
        ...
        self.detail_url = reverse(
                        "listing-detail",
                        kwargs={"pk": self.apartment.pk},)

    def test_anonymous_user_can_read(self):
        """
        Verify that an anonymous user can access
    ...
    def test_other_user_cannot_update_listing(self):
        """
        Verify that another user cannot modify
        ...
        self.assertEqual(
            self.apartment.title,
            "Hamburg Apartment",)

    def test_other_user_cannot_delete_listing(self):
        """
        Verify that another user cannot delete
        someone else's listing.
        """
